...
begin
    ...
    --MCR
    MCR: entity work.generic_register(reset_to_high)
         Generic Map(Bits=>5)
         Port Map(clk=>clock, reset=>reset, enable=>MCRle,
                  Data_in=>MCtrl, data_out=>MCR_out);
    control <= (MCR_out(4)) & (MCR_out(3)) & (MCR_out(2)) & (MCR_out(1)) &  (MCR_out(0));--active low    --remaining outputs
    flags <= CCR_out;
end Behavioral;
